...
// TODO Add batch processing of Google Places Scrape (city by city, because this script requires extended PHP max_execution_time)

// All main Google Places Categories
...
if (isset($_POST['submit']) && isset($_POST['place_category'])) {
    ...
    if (!file_exists($directory)) {
        ...
        debug_log("exported_csv folder has been created!");
    }


    // Declare path to save CSV in
    $slugified_place_category = sanitizeForCsv($_POST['place_category']);
    ...
    foreach ($cities as $city) {
        ...
        debug_log("Curently checking: {$_POST['place_category']} in $city");

        $places = fetchUrl($google_places_base_search_url, $searchParams);

        if (isset($places["results"])) {
            foreach ($places["results"] as $place) {
                debug_log("Checking place: {$place['place_id']}");
                ...
                if($details["result"]["types"][0] == $_POST['place_category']) $rowData = [
                    "Name" => sanitizeForCsv($details["result"]["name"] ?? ''),
                    "Phone" => sanitizeForCsv($details["result"]["formatted_phone_number"] ?? ''),
                    "Address" => sanitizeForCsv($details["result"]["formatted_address"] ?? ''),
                    "Hours" => sanitizeForCsv(implode(", ", $details["result"]["opening_hours"]["weekday_text"] ?? [])),
                    "Website" => sanitizeForCsv($details["result"]["website"] ?? ''),
                    "Review 1" => sanitizeForCsv($reviews[0]["text"] ?? ''),
                    "Review 2" => sanitizeForCsv($reviews[1]["text"] ?? ''),
                    "Review 3" => sanitizeForCsv($reviews[2]["text"] ?? ''),
                    "Review 4" => sanitizeForCsv($reviews[3]["text"] ?? ''),
                    "Review 5" => sanitizeForCsv($reviews[4]["text"] ?? ''),
                    "Review 6" => sanitizeForCsv($reviews[5]["text"] ?? ''),
                    "Type" => sanitizeForCsv(implode(", ", $details["result"]["types"] ?? [])),
        ...
        } else debug_log("No place has been returned");
    }
    ...
    // header("Content-Disposition: attachment; filename=$slugified_place_category.csv");

    $output = fopen($output_path, "w");

    if ($output === false) {
        debug_log("Failed to open the CSV file for writing.");
    ...
    debug_log("Google Places Scrape has finished!\n");
