<?php
    ...
    $post_content .= '<div class="wp-block-columns are-vertically-aligned-center is-layout-flex wp-container-3 wp-block-columns-is-layout-flex">';
        ...
        $post_content .= '<div class="wp-block-column is-vertically-aligned-center is-layout-flow wp-block-column-is-layout-flow">';
            ...
            // INFO LIST
                ...
                $post_content .= (!empty($phone)) ? "<li><span>Telefon: $phone</span></li>";
                $post_content .= (!empty($URLItem)) ? "<li><span>Strona internetowa: $URLItem</span></li>";
        ...
        // WEBSITE SCREENSHOT
            $post_content .= '<div class="wp-block-column is-vertically-aligned-center is-layout-flow wp-block-column-is-layout-flow">';
                $post_content .= '<figure class="wp-block-image size-large"><img decoding="async" src="' . wp_get_attachment_image_url( $placeholder_id, 'large' ) . '" alt=""></figure>';
            $post_content .= '</div>';
        // -----------------------------------------
    ...
    // TODO Remove " and ' signs from reviews
        $post_content .= '<div class="wp-block-group is-content-justification-center is-nowrap is-layout-flex wp-container-4 wp-block-group-is-layout-flex">';
            ...
            foreach($reviewItems as $reviewItem){
                $post_content .= "<div class='cts_review_item'>" . $reviewItem . "</div>";
            }
            $post_content .= '</div>';
        $post_content .= '</div>';
    // -----------------------------------------
    ...
    // GOOGLE STATIC MAPS API IMAGE
        $post_content .= "<img class='ctp_map_image' scr='$signedUrl' alt='$pretty_place_name'>";
    // -----------------------------------------